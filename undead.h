...
public:
    UndeadAdapter(shared_ptr<Undead> u) { 
        ...
        if(undead->name() == "Zombie") type = CharacterType::Zombie;
               else if (undead->name() == "Skeleton") type = CharacterType::Skeleton;
               else if (undead->name() == "Lich") type = CharacterType::Lich;
